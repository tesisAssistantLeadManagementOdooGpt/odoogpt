...
templateXML = os.path.join(module_dir, 'data')
#loader = FileSystemLoader(templateXML)
#env = Environment(loader=loader)
#template = env.get_template('data.txt')
#raise ValidationError(_('OpenAI Api Token not set in settings!', str(templateXML)))

_logger = logging.getLogger(__name__)


openai.api_key_path = templateXML+str("/data.txt")
...
class OdoogptOpenaiUtils(models.AbstractModel):
    ...
    def _models_list__get_parameters(self):
        _logger.info("Variable ENZO")
        _logger.info(templateXML)
        """Get params to pass to openai library to make a successful request"""
    ...
    def fine_tunes_cancel(self, fine_tune_id, **kwargs):
        ...
        response = openai.FineTuningJob.cancel(
            id=fine_tune_id,
            **{
